...
const testimonials = [
  {
    quote:
      "This platform has completely transformed how our club manages leagues. Member communication is seamless, and enrollment has never been easier.",
  ...
  {
    quote:
      "I love being able to see all my league stats and schedules in one place. It's made it so much easier to stay organized across three different clubs.",
    author: "Mike Chen",
  ...
  {
    quote:
      "The notification system is a game-changer. We can instantly reach all our members with updates about court closures or schedule changes.",
...
function TestimonialCard({
  ...
  return (
    <div
      className={`group relative flex flex-col gap-lg p-xl 
    ...
    >
        ...
        <div className="flex gap-xs" aria-label={`${rating} out of 5 stars`}>
          {[...Array(5)].map((_, i) => (
            <Icon
              ...
              key={i}
              className={`w-5 h-5 ${
                i < rating ? "fill-primary text-primary" : "text-outline"
              }`}
            />
          ))}
        </div>
      ...
      <div className="flex items-center gap-md pt-md">
        ...
        <div
          className={`w-12 h-12 rounded-full ${colors.avatarBg} ${colors.avatarText} 
