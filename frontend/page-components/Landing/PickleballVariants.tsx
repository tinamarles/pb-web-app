// frontend/app/page-components/Landing/PickleballVariants.tsx
...
export const BluePickleball = memo(function BluePickleball({
  ...
  return (
    ...
    >
      <defs>
        <radialGradient id={`blueGradient-${size}`} cx="30%" cy="30%">
      ...
      {holes.map((hole, index) => {
        ...
        const perspectiveFactor = (radius * 0.85 - hole.z) / (radius * 0.85); // 0 = front, 1 = edge
        ...
        if (perspectiveFactor > edgeThreshold) {
          ...
          const stretch = 1.2 + (perspectiveFactor - edgeThreshold) * 0.3; // Subtle stretch
...
export const YellowPickleball = memo(function YellowPickleball({
  ...
  return (
    ...
    >
      <defs>
        <radialGradient id={`yellowGradient-${size}`} cx="30%" cy="30%">
          <stop offset="0%" stopColor="#fff8dc" />
          <stop offset="70%" stopColor="#ffd700" />
          <stop offset="100%" stopColor="#daa520" />
        </radialGradient>

        <radialGradient id="yellowHoleGradient" cx="30%" cy="30%">
      ...
      {holes.map((hole, index) => {
        const perspectiveFactor = (radius * 0.85 - hole.z) / (radius * 0.85);
        const edgeThreshold = 0.7;

        if (perspectiveFactor > edgeThreshold) {
          const stretch = 1.2 + (perspectiveFactor - edgeThreshold) * 0.3;
          const radialAngle = Math.atan2(hole.y, hole.x);
...
export const OrangePickleball = memo(function OrangePickleball({
  ...
  return (
    ...
    >
      <defs>
        <radialGradient id={`orangeGradient-${size}`} cx="30%" cy="30%">
          <stop offset="0%" stopColor="#ffe4b5" />
          <stop offset="70%" stopColor="#ff8c00" />
          <stop offset="100%" stopColor="#ff6347" />
        </radialGradient>

        <radialGradient id="orangeHoleGradient" cx="30%" cy="30%">
...
export const GreenPickleball = memo(function GreenPickleball({
  ...
  return (
    ...
    >
      <defs>
        <radialGradient id={`greenGradient-${size}`} cx="30%" cy="30%">
          <stop offset="0%" stopColor="#f0fff0" />
          <stop offset="70%" stopColor="#32cd32" />
          <stop offset="100%" stopColor="#228b22" />
        </radialGradient>

        <radialGradient id="greenHoleGradient" cx="30%" cy="30%">
...
export const PickleballVariant = memo(function PickleballVariant({
  variant,
  size = 120,
  className = "",
  ...
  switch (variant) {
    case "blue":
      return <BluePickleball size={size} className={className} />;
    case "yellow":
      return <YellowPickleball size={size} className={className} />;
    case "orange":
      return <OrangePickleball size={size} className={className} />;
    case "green":
      return <GreenPickleball size={size} className={className} />;
    default:
      return <BluePickleball size={size} className={className} />;
  }
});
