...
function HeaderSectionSwitch() {
  ...
  .header {
    ...
    @apply flex items-center w-full gap-sm;
  ...
  */
  ...
  return (
    <>
      {/* Navigation Bar */}
      <nav className="sticky top-0 z-50 flex items-center  w-full gap-4 bg-background border-b border-outline-variant shrink-0 h-16">
        <div className="max-w-screen-2xl flex flex-wrap items-center justify-between mx-auto p-4">
          ...
          {/* Menu */}
          <div className="hidden lg:flex items-center space-x-6 text-md font-medium">
            {links.map((link, index) => (
              ...
              >
                <span className="block py-2 px-3 text-on-surface rounded-sm hover:bg-primary/80">
                  {link.name}
                </span>{" "}
              </Link>
            ))}
          ...
          <div className="flex lg:order-2 space-x-3 rtl:space-x-reverse items-center">
            ...
            >
              <Bars3Icon className="h-6 w-6 rounded-full text-on-surface hover:text-primary transition-colors" />
            </button>
          </div>
        </div>
      </nav>
