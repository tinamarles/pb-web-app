...
export const CustomSelect = memo(function CustomSelect({
  ...
  return (
    <div ref={containerRef} className={`${icon ? 'input-field' : 'relative'} ${className}`}>
      ...
      {icon && (
        <Icon name={icon} className="icon-lg text-on-surface-variant" />
      )}
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <Icon name='chevrondown' 
            className={`icon-lg text-on-surface-variant transition-transform duration-200 ${
              isOpen ? "rotate-180" : ""
            }`} 
          />
        </div>
